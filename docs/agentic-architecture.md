...
- "Fix ClickHouse lag" was classified as S1_2 (Support), corrected to S1_3 (Query Service)
- "ADOM pending check" was classified as S0_0 (Infrastructure), corrected to S1_0 (Query Service)
```
...
```
Correction: "ADOM pending" → Query Service  (1st time: recorded)
Correction: "ADOM pending" → Query Service  (2nd time: auto-glossary triggered)
...
→ Suggestion 1: "ClickHouse items always go to Query Service"
   Action: Add glossary term "clickhouse" → S1_0
   [Apply] [Dismiss]
