...
func TestFormatItemDescriptionCapitalization(t *testing.T) {
	itemWithAuthor := TemplateItem{
		Author:      "alex stone",
		Description: `set heavyInfologTimescaleDBJdbcTemplate to "not required"`,
		Status:      "in progress",
	}
	gotTeam := formatTeamItem(itemWithAuthor)
	wantTeam := `**Alex Stone** - Set heavyInfologTimescaleDBJdbcTemplate to "not required" (in progress)`
	if gotTeam != wantTeam {
